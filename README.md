...
Afterwards please reference the actual `README.md` you just created in your new project folder, all the references to "kevin" will be changed accordingly.
...
*Prerequisites: virtualenv, virtualenvwrapper*
    ...
    cd kevin
    mkvirtualenv kevin-dev && add2virtualenv `pwd`
    mkvirtualenv kevin-prod && add2virtualenv `pwd`
    mkvirtualenv kevin-test && add2virtualenv `pwd`
...
This will compile all the files in `/kevin/static` for the first run.
...
After `post_compile` is successful, uncomment the line with the variable `STATICFILES_STORAGE` in `/kevin/config/settings/base.py` to enable django-pipeline and push again.
...
**WARNING**: If this project has SSL turned on, [localhost:5000](http://localhost:5000) won't work anymore because it will always try to redirect to [https://localhost:5000](https://localhost:5000). To fix this comment out the SECURITY CONFIGURATION section in `/kevin/config/settings/prod.py`
...
*`.env.test` is not version controlled so the first person to create this project needs to create a `.env.test` file for Foreman to read into the environment. Future collaboraters need to email the creator for it.*
    ...
    echo PYTHONPATH=kevin >> .env.test
...
Grunt automatically compiles Jasmine tests written in CoffeeScript at `/kevin/static/js/tests/coffee` and runs the tests upon every save.
...
SSL
---
Enable SSL via Heroku, Cloudflare, or your DNS provider and then uncomment the SECURITY CONFIGURATION section in `/kevin/config/settings/prod.py` to enable security best practices for production.
...
Add the following config variables to Heroku:
    ...
    heroku config:set AWS_STATIC_STORAGE_BUCKET_NAME=kevin-static
    heroku config:set AWS_MEDIA_STORAGE_BUCKET_NAME=kevin-media
